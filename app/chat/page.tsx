  ...
  return (
    <div className="h-dvh flex flex-col mesh-bg">
      ...
      <div className="glass px-4 py-3 flex items-center justify-between z-10" style={{ borderTop: 'none', borderLeft: 'none', borderRight: 'none', borderRadius: 0 }}>
        <button onClick={() => router.push('/result')} className="text-[#6B7194] text-sm flex items-center gap-1 transition-colors hover:text-[#1A1D2E] min-w-[44px] min-h-[44px] -ml-2 pl-2 rounded-xl">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          Back
        </button>
        <h1 className="text-sm font-semibold text-[#1A1D2E] flex items-center gap-1.5">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
        {messages.length === 0 && (
          <div className="text-center text-[#6B7194] text-sm mt-8 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-4">
              {['Is this good for diabetics?', 'What are the harmful ingredients?', 'Give me better alternatives'].map((q) => (
                <button
                  ...
                  className="text-xs glass-subtle text-[#3A3F57] px-3.5 py-2.5 rounded-full transition-all hover:bg-white hover:shadow-sm min-h-[44px]"
                  style={{ borderColor: 'rgba(26, 29, 46, 0.08)' }}
                >
                  {q}
                </button>
              ))}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="glass text-[#F1B123] px-5 py-3 rounded-2xl rounded-bl-md text-sm" style={{ borderColor: 'rgba(241, 177, 35, 0.12)' }}>
              <div className="flex gap-1.5">
                <span className="w-2 h-2 bg-[#F1B123] rounded-full animate-bounce" />
                <span className="w-2 h-2 bg-[#F1B123] rounded-full animate-bounce" style={{ animationDelay: '0.15s' }} />
                <span className="w-2 h-2 bg-[#F1B123] rounded-full animate-bounce" style={{ animationDelay: '0.3s' }} />
              </div>
            </div>
          </div>
        )}
      ...
      <div className="glass px-4 py-3 flex gap-2" style={{ borderBottom: 'none', borderLeft: 'none', borderRight: 'none', borderRadius: 0 }}>
        <input
          ...
          className="flex-1 glass-subtle rounded-2xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#F1B123]/30 transition-all"
          style={{ borderColor: 'rgba(26, 29, 46, 0.06)' }}
        />
        <button
          onClick={() => sendMessage()}
          disabled={loading || !input.trim()}
          className="w-11 h-11 rounded-2xl btn-gold flex items-center justify-center disabled:opacity-40 shrink-0"
        >
...
export default function ChatPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center mesh-bg"><div className="w-10 h-10 border-2 border-[#F1B123] border-t-transparent rounded-full animate-spin" /></div>}>
      <ChatContent />
    </Suspense>
  )
}
